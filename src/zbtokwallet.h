...
#ifndef BEETOK_ZBTOKWALLET_H
#define BEETOK_ZBTOKWALLET_H
...
public:
    CzBTOKWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBTOK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBTOK(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BEETOK_ZBTOKWALLET_H
